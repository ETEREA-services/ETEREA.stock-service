...
class WikiGenerationError(Exception):
    """Excepción específica para errores en la generación de la wiki"""
    pass

def run_command(cmd, check=True):
    """Ejecuta un comando con manejo mejorado de errores"""
...
def get_default_branch(wiki_url, headers):
    """Determina la rama predeterminada de la wiki"""
    try:
        ...
        pass
    return 'main'  # Valor predeterminado si no se puede determinar

def run_git_command(cmd, error_msg=None, check=True):
    """Ejecuta un comando git con manejo mejorado de errores"""
    try:
        result = run_command(cmd, check=False)  # Siempre False para manejar errores manualmente
        if check and result.returncode != 0:  # Solo verificar si check=True
            if 'Authentication failed' in result.stderr:
                raise WikiGenerationError("Error de autenticación en Git. Verifica el token de acceso.")
            elif 'repository not found' in result.stderr:
                raise WikiGenerationError("Repositorio wiki no encontrado. Verifica que la wiki esté habilitada.")
            elif error_msg:
        ...
        if not isinstance(e, WikiGenerationError):
            raise WikiGenerationError(f"Error al ejecutar comando Git: {str(e)}")
        raise

def verify_token():
    """Verifica la existencia y formato del token de GitHub"""
    token = os.environ.get('GITHUB_TOKEN')
    if not token:
        raise WikiGenerationError("Token de GitHub no encontrado en variables de entorno")
    if not token.strip():
        raise WikiGenerationError("Token de GitHub está vacío")
    return token
...
def generate_wiki_pages():
    ...
    print(f"Iniciando proceso de generación de Wiki desde {original_dir}")
